...
- **Renderer stack:** React 19, TanStack Router (file-based routes in `src/renderer/routes`,
  hash history; `routeTree.gen.ts` is generated by the Vite plugin during `npm run build` and
  committed so typecheck works without a build; a detail route next to a list route uses the
