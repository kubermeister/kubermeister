...
- A detail route next to a list route uses the `list_.$name.{-$tab}.tsx` naming so it renders as a
...
- Adding a kind:
  ...
  - a `list/index.tsx` plus `list/$namespace.$name.{-$tab}.tsx` route pair;
...
- **An edit on disk takes effect while the app runs.** `watchSettingsFile` polls the file with
  ...
  remade.
- `settings.schema.json` at the root is the published JSON Schema for the file, generated by
  `settingsJsonSchema` (`src/shared/settings-schema.ts`) from the same zod schemas the app reads the
  file with. `npm run schema` regenerates it, `tests/unit/repo/settings-schema.test.ts` fails when
  the committed file drifts, and a new key fails that test until it has a line in
  `KEY_DESCRIPTIONS`. A file the app creates starts with `$schema` pointing at it; one somebody
  wrote is never given one.
