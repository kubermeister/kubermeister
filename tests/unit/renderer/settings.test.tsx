...
describe('settings screen', () => {
    ...
    it('writes the quit confirmation through the bridge', async () => {
        renderRoutes(routeTree, '/settings');
        const toggle = await screen.findByRole('switch', { name: 'Confirm before quitting' });
        await waitFor(() => expect(toggle).toHaveAttribute('aria-checked', 'true'));
        await userEvent.click(toggle);
        await waitFor(() => expect(invoke).toHaveBeenCalledWith('settings.set', { general: { confirmQuit: false } }));
        await waitFor(() => expect(toggle).toHaveAttribute('aria-checked', 'false'));
    });
